...
public class VfxCommonGUI : ShaderGUI
{
    ...
    public void FindProperties(MaterialProperty[] props)
    {
        ...
        // [新增] 绑定 Shader 中的 _UseCustom2 属性
        UseCustom2 = FindProperty("_UseCustom2", props);
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        if(SetFold= Foldouts(SetFold, "渲染设置",ref setingColor))
        {
            ...
            // [新增] 绘制 CustomData2 开关
            if (UseCustom2 != null)
            {
                EditorGUILayout.Space();
                m_MaterialEditor.ShaderProperty(UseCustom2, "启用 CustomData2 偏移 (主贴图/遮罩)");
                Introduce("注意：如果不启用此项，即使粒子面板输入了Custom2数据，主贴图和遮罩偏移也不会生效。");
                EditorGUILayout.Space();
            }
