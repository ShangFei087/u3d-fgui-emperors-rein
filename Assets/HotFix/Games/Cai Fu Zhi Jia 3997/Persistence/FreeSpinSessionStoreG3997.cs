...
{
    public class FreeSpinSessionStoreG3997
    {
        /// <summary>
        /// 当前游戏 ID（财富之家 3997）。
        /// </summary>
        const int GameId = 3997;

        /// <summary>
        /// 生成免费局会话存档键，按「会话版本 + 游戏 ID + 玩家 ID」区分。
        /// </summary>
        static string BuildKey(int playerId) =>
            $"SlotFreeSpinSession_v{FreeSpinSessionSnapshotG3997.CurrentSessionVersion}_{GameId}_{playerId}";
        ...
        {
            var cm = ContentModel.Instance;
            return cm.FreeSpinTotalTimes > 0 && cm.FreeSpinPlayTimes >= cm.FreeSpinTotalTimes &&
        ...
        public static void TryPersistOrClearSession()
        {
            ...
            var cm = ContentModel.Instance;
            var snap = new FreeSpinSessionSnapshotG3997
            {
                PlayerId = pid,
                FreeSpinTotalTimes = cm.FreeSpinTotalTimes,
                FreeSpinPlayTimes = cm.FreeSpinPlayTimes,
                FreeSpinTotalWinCredit = cm.freeSpinTotalWinCoins,
        ...
        public static FreeSpinSessionSnapshotG3997 TryLoad(int playerId)
        {
            if (ApplicationSettings.Instance.isMock)
                return null;
            ...
            try
            {
                var snap = JsonConvert.DeserializeObject<FreeSpinSessionSnapshotG3997>(json);
                if (snap == null || snap.SessionVersion != FreeSpinSessionSnapshotG3997.CurrentSessionVersion)
                    return null;
