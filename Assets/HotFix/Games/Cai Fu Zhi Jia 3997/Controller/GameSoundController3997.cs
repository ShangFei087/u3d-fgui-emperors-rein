...
using CustomModel = CaiFuZhiJia_3997.CustomModel;
using SoundKey = CaiFuZhiJia_3997.SoundKey;

namespace CaiFuZhiJia_3997
{
    public static class Game3997AudioEvent
    {
        /// <summary> 主游戏循环 BGM </summary>
        public const string BgmRegularGame = "3997_BgmRegularGame";

        /// <summary> 免费局内循环 BGM </summary>
        public const string BgmFreeSpinGame = "3997_BgmFreeSpinGame";

        /// <summary> 彩金小游戏局内循环 BGM </summary>
        public const string BgmBonusGame = "3997_BgmBonusGame";

        /// <summary> 免费触发弹窗 BGM </summary>
        public const string BgmFreeSpinTrigger = "3997_BgmFreeSpinTrigger";

        /// <summary> 免费结算弹窗 BGM </summary>
        public const string BgmFreeSpinResult = "3997_BgmFreeSpinResult";

        /// <summary> 彩金触发弹窗 BGM </summary>
        public const string BgmBonusTrigger = "3997_BgmBonusTrigger";
    ...
    public class GameSoundController3997
{
    ...
    public GameSoundController3997()
    {
        Init();
    }
    ...
    private void PlayScatterBonusColumnStopSoundsFromFlags(int column0Based, bool hasScatter, bool hasBonus)
    {
        ...
        if (hasScatter)
            GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.ScatterDown);

        if (hasBonus)
            GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.BonusDown1);
    }
    ...
    private void OnSlotEvent(EventData receivedEvent)
    {
        switch ((string)receivedEvent.name)
        {
            case SlotMachineEvent.SpinSlotMachine:
                ...
                //     GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.BonusRolling);
                // else
                //     GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.NormalRolling);
                break;
            case SlotMachineEvent.StoppedSlotMachine:
                ...
                if (GameSoundHelper3997.Instance.IsPlaySound(SoundKey.FreeRollingBox))
                    GameSoundHelper3997.Instance.StopSound(SoundKey.FreeRollingBox);
                if (GameSoundHelper3997.Instance.IsPlaySound(SoundKey.BonusRollingBox))
                    GameSoundHelper3997.Instance.StopSound(SoundKey.BonusRollingBox);
                break;
        }
    }
    ...
    private void OnSlotDetailEvent(EventData receivedEvent)
    {
        ...
        if (name == SlotMachineEvent.ReelColumnStopSound)
        {
            ...
            GameSoundHelper3997.Instance.PlaySoundEff(key);
            return;
        }
    ...
    private void OnAudioEvent(EventData receivedEvent)
    {
        switch ((string)receivedEvent.name)
        {
            case SlotMachineEvent.FreeGameFadeTransition:
                GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.FadeFree);
                break;
            case SlotMachineEvent.BonusGameFadeTransition:
                GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.FadeBonus);
                break;
            case SlotMachineEvent.FreeSpinPopupAppear:
                GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.FreePopupAppear);
                break;
            case SlotMachineEvent.FreeSpinPopupDisappear:
                GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.FreePopupDisappear);
                break;
            case SlotMachineEvent.JackpotPopupAppear:
                GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.BonusPopupAppear);
                break;
            case SlotMachineEvent.JackpotPopupDisappear:
                GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.BonusPopupDisappear);
                break;
            ...
            case SlotMachineEvent.FreeRollingBox:
                ...
                if (GameSoundHelper3997.Instance.IsPlaySound(SoundKey.FreeRollingBox))
                    GameSoundHelper3997.Instance.StopSound(SoundKey.FreeRollingBox);
                GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.FreeRollingBox);
                break;
            case SlotMachineEvent.BonusRollingBox:
                if (GameSoundHelper3997.Instance.IsPlaySound(SoundKey.BonusRollingBox))
                    GameSoundHelper3997.Instance.StopSound(SoundKey.BonusRollingBox);
                GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.BonusRollingBox);
                break;
            case Game3997AudioEvent.BgmRegularGame:
                GameSoundHelper3997.Instance.PlayMusicSingle(SoundKey.RegularBG);
                break;
            case Game3997AudioEvent.BgmFreeSpinGame:
                GameSoundHelper3997.Instance.PlayMusicSingle(SoundKey.FreeSpinBG);
                break;
            case Game3997AudioEvent.BgmBonusGame:
                GameSoundHelper3997.Instance.PlayMusicSingle(SoundKey.BonusBG);
                break;
            case Game3997AudioEvent.BgmFreeSpinTrigger:
                GameSoundHelper3997.Instance.PlayMusicSingle(SoundKey.FreeSpinTriggerBG);
                break;
            case Game3997AudioEvent.BgmFreeSpinResult:
                GameSoundHelper3997.Instance.PlayMusicSingle(SoundKey.FreeSpinResultBG);
                break;
            case Game3997AudioEvent.BgmBonusTrigger:
                GameSoundHelper3997.Instance.PlayMusicSingle(SoundKey.BonusTriggerBG);
                break;
            case Game3997AudioEvent.BgmBonusResult:
                GameSoundHelper3997.Instance.PlayMusicSingle(SoundKey.BonusResultBG);
                break;
        }
    }
    ...
    private void OnTotalWinCreditForWinLevel(EventData receivedEvent)
    {
        ...
        {
            winCredit = ContentModel.Instance.baseGameWinCredit;
        }

        if (ContentModel.Instance.IsBonusTrigger) return; // 彩金游戏触发期间，结束是在弹窗结束之后
        if (winCredit <= 0) return;

        long totalBet = ContentModel.Instance.totalBet;
        if (totalBet <= 0)
        {
            GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.win_lv1);
            return;
        }

        // ≥3 倍押注高档，≥2 倍中档，否则低档
        if (winCredit >= 3L * totalBet)
            GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.win_lv3);
        else if (winCredit >= 2L * totalBet)
            GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.win_lv2);
        else
            GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.win_lv1);
    }
    ...
    private void TryPlayScatterWinSound()
    {
        ...
        GameSoundHelper3997.Instance.PlaySoundEff(SoundKey.ScatterWin);
    }
