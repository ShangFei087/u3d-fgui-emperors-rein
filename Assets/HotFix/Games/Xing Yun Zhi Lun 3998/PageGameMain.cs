...
namespace XingYunZhiLun_3998
{
    ...
    public class PageGameMain : MachinePageBase
    {
        ...
        private Transition hideZhuanPan, showZhuanPan;
        ...
        public void InitParam(EventData data)
        {
            ...
            MainModel.Instance.gameID = 3998;
            MainModel.Instance.gameName = "XinYunZhiLun_3998";
            MainModel.Instance.displayName = "XinYunZhiLun_3998";
            ...
            if (gWheelFrame != loadWheelFrame)
            {
                ...
                upEffect = wheelObj.transform.GetChild(1).GetChild(0);

                GameCommon.FguiUtils.AddWrapper(gWheelFrame, wheelObj);

                ChangeParent(gWheel, wheelObj, "Anchor/Spine Mecanim GameObject (Lucky_ng_img_turntable)/SkeletonUtility-SkeletonRoot/root/c_circle");
            }
        ...
        void TryRestoreFreeSpinSession()
        {
            ...
            {
                FreeSpinSessionStoreG3998.Clear(pid);
                return;
            }
            ...
            cm.freeSpinAddNum = 0;
            cm.freeWildRecord = snap.freeWildRecord;
            ...
            DebugUtils.Log($"[G3998] 已恢复免费局快照：剩余 {cm.showFreeSpinRemainTime} / 总 {cm.freeSpinTotalTimes}，待首局 Spin 与算法校验。");
        }



        long allFreeWinCredit = 0;
        //免费游戏触发
        ...
        IEnumerator GameFreeSpinOnce(Action successCallback, Action<string> errorCallback)
        {
            ...
            if (winList.Count > 0)
            {
                ...
                if (winLevelType != WinLevelType.None)
                {
                    bool isBigWinFinish = false;

                    //bigwin弹窗
                    yield return BigWinPopup(winLevelType, ContentModel.Instance.baseGameWinCredit, () => isBigWinFinish = true);

                    yield return new WaitUntil(() => isBigWinFinish == true);
                    slotMachineCtrl.CloseSlotCover();
                    slotMachineCtrl.SkipWinLine(false);
                }
            }
        ...
        private void JackpotBgOpen()
        {
            ...
            if(startTransition == null && initTransition == null)
            {
                ...
                initTransition.ignoreEngineTimeScale = false;
            }
        ...
        public float itemWidth = 555f;        // 单个Item宽度
        public float itemSpacing = 110f;      // Item间距
        public float itemTotalWidth = 665f;   // 总Item宽度（包括间距）
        public float listWidth = 1890f;       // 列表总宽度
        ...
        private void RenderListItem(int index, GObject obj)
        {
            ...
            if (sorceTextObj != null)
            {
                ...
                textField.text = (UnityEngine.Random.Range(40, 120) * 100).ToString();
            }
        ...
        private string GetIconIndexForItem(int index, out bool isWinner)
        {
            ...
            // 其他索引随机
            return Random.Range(0, 7).ToString();
        }
        ...
        {
            if (ContentModel.Instance.isJackpotWin) return ContentModel.Instance.jackpotType;
        ...
        private void ChangeParent(GObject gComponent, GameObject go, string path)
        {
            ...
            if (gComponent.displayObject?.gameObject != null)
            {
                ...
                t.localPosition = new Vector3(-275.5f, 275.9f, 0);
        ...
        private IEnumerator ChangeWheel()
        {
            switch (ContentModel.Instance.scatterCount)
            {
                case 4:
                    PlayAnim(wheelAnim, "01_02");
                    yield return new WaitForSeconds(0.3f);
                    PlayEffectAnim(upEffect);
                    ChangeWheelURL(1);
        ...
        private IEnumerator ReturnWheel()
        {
            switch (ContentModel.Instance.scatterCount)
            {
                case 4:
                    PlayAnim(wheelAnim, "02_01");
                    ChangeWheelURL(0);
                    yield return new WaitForSeconds(1);
                    break;
                case 5:
