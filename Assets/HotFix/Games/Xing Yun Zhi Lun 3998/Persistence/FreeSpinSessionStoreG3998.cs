...
{
    public static class FreeSpinSessionStoreG3998
    {
        /// <summary>
        /// 当前游戏 ID（朱再金币 1700）。
        /// </summary>
        const int GameId = 3998;

        /// <summary>
        /// 生成免费局会话存档键，按「会话版本 + 游戏 ID + 玩家 ID」区分。
        /// </summary>
        static string BuildKey(int playerId) => $"SlotFreeSpinSession_v{FreeSpinSessionSnapshotG3998.CurrentSessionVersion}_{GameId}_{playerId}";
        ...
        public static void TryPersistOrClearSession()
        {
            ...
            var cm = ContentModel.Instance;
            var snap = new FreeSpinSessionSnapshotG3998
            {
                PlayerId = pid,
        ...
        public static FreeSpinSessionSnapshotG3998 TryLoad(int playerId)
        {
            if (ApplicationSettings.Instance.isMock)
                return null;
            ...
            try
            {
                var snap = JsonConvert.DeserializeObject<FreeSpinSessionSnapshotG3998>(json);
                if (snap == null || snap.SessionVersion != FreeSpinSessionSnapshotG3998.CurrentSessionVersion)
                    return null;
            ...
            {
                Debug.LogError($"[G3998] FreeSpinSession 反序列化失败: {e.Message}");
                return null;
            }
        }
