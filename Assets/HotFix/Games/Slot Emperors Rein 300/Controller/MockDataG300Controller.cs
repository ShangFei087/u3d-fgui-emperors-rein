...
namespace SlotEmperorsRein
{
    ...
    public class MockDataG300Controller : Singleton<MockDataG300Controller>
    {
        public bool isBouns;
        ...
        List<string[]> BonusdatasSpin = new List<string[]>()
        {
            new string[] { "Assets/HotFix/Games/_Mock/Resources/G200/g200__slot_spin__bonus1.json" },
            new string[] { "Assets/HotFix/Games/_Mock/Resources/G200/g200__slot_spin__bonus2.json" },
            new string[] { "Assets/HotFix/Games/_Mock/Resources/G200/g200__slot_spin__bonus3.json" },
            new string[] { "Assets/HotFix/Games/_Mock/Resources/G200/g200__slot_spin__bonus4.json" },
        };
            ...
            Action<BagelCodeError> errorCallback)
        {
            Timer.DelayAction(0.2f, () =>
            {
                if (curDatas.Count == 0)
                {
                    ...
                    if (isBouns)
                    {
                        strs = BonusdatasSpin[UnityEngine.Random.Range(0, 4)];
                    }
        ...
        public void RequestBonusSlotSpin(long totalBet, Action<JSONNode> successCallback,
           Action<BagelCodeError> errorCallback)
        {
            Timer.DelayAction(0.2f, () =>
            {
                if (curDatas.Count == 0)
                {
                    List<string[]> target = BonusdatasSpin; // 选普通游戏 还是免费游戏

                    string[] strs = target[UnityEngine.Random.Range(0, BonusdatasSpin.Count)];
                    //UnityEngine.Random.Range(0, target.Count)
                    curDatas = new Queue<string>(strs);
                }
