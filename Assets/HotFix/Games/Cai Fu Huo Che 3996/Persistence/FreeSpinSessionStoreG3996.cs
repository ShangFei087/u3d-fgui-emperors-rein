...
namespace CaiFuHuoChe_3996
{
    public static class FreeSpinSessionStoreG3996
    {
        ...
        /// 当前游戏 ID（朱再金币 1700）。
        /// </summary>
        const int GameId = 3996;

        /// <summary>
        /// 生成免费局会话存档键，按「会话版本 + 游戏 ID + 玩家 ID」区分。
        /// </summary>
        static string BuildKey(int playerId) => $"SlotFreeSpinSession_v{FreeSpinSessionSnapshotG3996.CurrentSessionVersion}_{GameId}_{playerId}";
        ...
        public static void TryPersistOrClearSession()
        {
            ...
            var cm = ContentModel.Instance;
            var snap = new FreeSpinSessionSnapshotG3996
            {
                PlayerId = pid,
        ...
        public static FreeSpinSessionSnapshotG3996 TryLoad(int playerId)
        {
            if (ApplicationSettings.Instance.isMock)
                return null;
            ...
            try
            {
                var snap = JsonConvert.DeserializeObject<FreeSpinSessionSnapshotG3996>(json);
                if (snap == null || snap.SessionVersion != FreeSpinSessionSnapshotG3996.CurrentSessionVersion)
                    return null;
