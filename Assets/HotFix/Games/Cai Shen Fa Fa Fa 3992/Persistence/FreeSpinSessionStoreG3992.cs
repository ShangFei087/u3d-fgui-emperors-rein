...
{
    public class FreeSpinSessionStoreG3992
{
   /// <summary>
        /// 当前游戏 ID（财神发发发 3992）。
        /// </summary>
        const int GameId = 3992;

        /// <summary>
        /// 生成免费局会话存档键，按「会话版本 + 游戏 ID + 玩家 ID」区分。
        /// </summary>
        static string BuildKey(int playerId) => $"SlotFreeSpinSession_v{FreeSpinSessionSnapshotG3992.CurrentSessionVersion}_{GameId}_{playerId}";
        ...
        public static void TryPersistOrClearSession()
        {
            ...
            var cm = ContentModel.Instance;
            var snap = new FreeSpinSessionSnapshotG3992
            {
                PlayerId = pid,
        ...
        public static FreeSpinSessionSnapshotG3992 TryLoad(int playerId)
        {
            if (ApplicationSettings.Instance.isMock)
                return null;
            ...
            try
            {
                var snap = JsonConvert.DeserializeObject<FreeSpinSessionSnapshotG3992>(json);
                if (snap == null || snap.SessionVersion != FreeSpinSessionSnapshotG3992.CurrentSessionVersion)
                    return null;
            ...
            {
                Debug.LogError($"[G3993] FreeSpinSession 反序列化失败: {e.Message}");
                return null;
            }
        }
