...
{
    public class PageConsoleEventRecord : MachinePageBase
    {

        public const string pkgName = "ConsoleCoinPusher01";
        public const string resName = "PageConsoleEventRecord";
        ...
        {
            PageTitleManager.Instance.AddPageNode("事件记录");
        ...
        int dateIndex = 0;
        List<string> dropdownDateLst = new List<string>();
        ...
        void ClearPage()
        {
            ...
            // 返回按钮放到最后
            //back.visible = true;
            //back.parent.SetChildIndex(back, 0);
        }
        void InitLogInfo()
        {
        ...
        void OnDropdownChangedDate(int indexDate)
        {
            ...
            string sql2 = $"SELECT * FROM {ConsoleTableName.TABLE_LOG_EVENT_RECORD} WHERE DATE(DATETIME(created_at / 1000, 'unixepoch', 'localtime')) = '{dropdownDateLst[indexDate]}'"; //可以用
        ...
        void ShowDetailedRecord()
        {
            ...
            //goRecord.GetChildAt(curIndexMenuItemRecord).asCom.GetChild("value2").asRichTextField.text;
        }
