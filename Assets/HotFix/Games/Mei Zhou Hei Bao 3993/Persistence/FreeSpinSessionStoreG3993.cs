...
namespace MeiZhouHeiBao_3993
{
    public class FreeSpinSessionStoreG3993
    {
        ...
        /// 当前游戏 ID（美洲黑豹 3993）。
        /// </summary>
        const int GameId = 3993;

        /// <summary>
        /// 生成免费局会话存档键，按「会话版本 + 游戏 ID + 玩家 ID」区分。
        /// </summary>
        static string BuildKey(int playerId) => $"SlotFreeSpinSession_v{FreeSpinSessionSnapshotG3993.CurrentSessionVersion}_{GameId}_{playerId}";
        ...
        public static FreeSpinSessionSnapshotG3993 TryLoad(int playerId)
        {
            if (ApplicationSettings.Instance.isMock)
                return null;
            ...
            try
            {
                var snap = JsonConvert.DeserializeObject<FreeSpinSessionSnapshotG3993>(json);
                if (snap == null || snap.SessionVersion != FreeSpinSessionSnapshotG3993.CurrentSessionVersion)
                    return null;
